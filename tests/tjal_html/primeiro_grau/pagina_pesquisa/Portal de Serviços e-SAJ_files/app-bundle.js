...
require('block-ui');
require('unj-popup-modal');
require('unj-certificado');

(function ($) {

    var abrirPopupModalContato = function () {
        $.saj.popupModal($('#divopupContato'), {
            titulo: dict['label.contato'],
            altura: 255,
            largura: 610,
    ...
    var utilizarLinkEmail = function () {
        window.location = "mailto:'" + appEsajLayout.emailParaContato + "'";
    };

    var $linkContato = $('#linkContato');
    if (appEsajLayout.utilizarBotaoContato === 'true') {
        $linkContato.click(abrirPopupModalContato);
    } else {
        $linkContato.click(utilizarLinkEmail);
    }
...
},{"block-ui":2,"jquery":3,"unj-certificado":4,"unj-popup-modal":5}],2:[function(require,module,exports){
/*!
...
(function () {
    ...
    function setup($) {
        ...
        $.saj.popupModal = function (textoHtml, options) {
            popupModal(textoHtml, options);
        };
        $.saj.popupModalAjax = function (url, options) {
            popupModalAjax(url, options);
        };
        ...
        var onClose = function (el, opts) {
            recuperarElemento(el, opts);
            $.event.trigger({
                type: "fechandoModal",
                message: opts
            })
            opts.onPopupModalClose(opts);
        };
        ...
        var getOptions = function (options) {
            ...
            $.saj.closePopupModal = function () {
                if (typeof sajOpts.onBeforeClosePopupModal === "function") {
                    sajOpts.onBeforeClosePopupModal();
                }
                $.unblockUI(sajOpts);
        ...
        var popupModalAjax = function (url, options) {
            options = getOptions(options);
            var conteudoDiv = $('<div>');
            conteudoDiv.html('<b>' + dict['mensagem.aguarde'] + '</b>');
            ...
            $.ajax({
                ...
                success: function (transport) {
                    /*if ($(transport).find('#aba-certificado').length) {
                     location.reload()
                     } else {*/
                ...
                error: function () {
                    conteudoDiv.removeClass('modalProcessando');
                    conteudoDiv.html('<b>' + dict['mensagem.paginaNaoCarregada'] + '</b>');
                },
        ...
        var montaPopupDiv = function (conteudoDiv, options) {
            ...
            popupDiv = $('<div id="popupModalDiv" role="dialog" aria-label="' + options.sajOpts.titulo + '">');
        ...
        var montaRodapeDiv = function (options) {
            ...
            var botaoOK = $('<input type="button" class="spwBotaoDefault">');
            botaoOK.val('Ok')
        ...
        var ajustarTamanhoDivComScroll = function () {
            var divMensagem = $('#mensagemAlert');
            var cabecalho = $('#popupModalDiv').find('table').height();
            var rodape = $('.boxInternoSajModal.alinhamentoDireitaSajModal').height();
            var blockui = $('.blockUI.blockMsg.blockPage').height();
